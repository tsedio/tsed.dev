---
url: /api/specs/schema/types/decorators/common/decorator-schema.md
description: api documentation of Schema from @tsed/schema
---

## Usage

```typescript
import { Schema } from "@tsed/schema";
```

> See [/packages/specs/schema/src/decorators/common/schema.ts](https://github.com/tsedio/tsed/blob/v8.38.7/packages/specs/schema/src/decorators/common/schema.ts#L0-L0).

## Overview

```ts
function Schema<T>(partialSchema: Partial<JsonSchemaObject> | JsonSchema<T>): (...args: any[]) => any;
```

## Description

Write data formatted to JsonSchema.

## Example

```ts
@Schema({title: "test"})
class Model {
   @Schema({formatMinimum: "1987-10-24"})
   @Format("date")
   birthDate: Date
}
```

Will produce:

```json
{
  "type": "object",
  "title": "test",
  "properties": {
    "birthdate": {
       "type": "string",
       "format": "date",
       "formatMinimum": "1987-10-24"
    }
  }
}
```
