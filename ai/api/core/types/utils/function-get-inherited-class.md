---
title: getInheritedClass from @tsed/core
description: api documentation of getInheritedClass from @tsed/core
meta:
 - name: keywords
   description: api typescript node.js documentation getInheritedClass function
---
# getInheritedClass - @tsed/core

## Usage

```typescript
import { getInheritedClass } from "@tsed/core";
```

> See [/packages/core/src/utils/ancestorOf.ts](https://github.com/tsedio/tsed/blob/v8.38.7/packages/core/src/utils/ancestorOf.ts#L0-L0).

## Overview

```ts
function getInheritedClass(target: any): any;
```

<!-- Description -->

## Description

Returns the inherited class (ancestor) of the given target.

An alias for {@link ancestorOf}.
